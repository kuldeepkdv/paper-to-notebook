  ...
  return (
    <div className={props.className}>
      <motion.div
        ...
        className="flex flex-col gap-6 pb-6 bg-[#050505]"
      >
        {[
          ...new Array(2).fill(0).map((_, index) => (
            <React.Fragment key={index}>
              {props.testimonials.map(({ text, image, name, role }, i) => (
                <div className="p-10 rounded-3xl border border-white/10 shadow-lg shadow-[#8ad4ff]/10 max-w-xs w-full bg-white/5" key={i}>
